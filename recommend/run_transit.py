...
def _parse_kakao_subway_doc(doc: Dict) -> Tuple[str, str]:
    ...
    # 부산, 대구 등 다른 지역 도시철도 명칭을 먼저 확인하고 제외
    if any(city in category for city in ["부산", "대구", "광주", "대전"]):
        # 만약 수도권 언급도 있다면, 다른 지역은 무시하고 수도권 노선만 추출
        if "수도권" not in category:
            return name, ""

    # place_name과 category_name에서 모든 숫자 기반 호선 찾기 (e.g., 1호선, 9호선)
    full_text = name + " " + category
    lines = re.findall(r'(\d+호선)', full_text)
    
    if not lines:
        return name, ""

    # 여러 호선이 찾아졌을 경우, 숫자로 정렬하여 가장 낮은 번호의 호선을 우선으로 함
    unique_lines = sorted(list(set(lines)), key=lambda x: int(re.search(r'\d+', x).group()))
    
    # 가장 대표적인(번호가 빠른) 노선 1개만 선택
    return name, unique_lines[0]
...
def _enrich_transit_hints_fast(df: pd.DataFrame, kakao_key: str) -> pd.DataFrame:
    ...
    def get_transit_info_for_tile(lat, lon):
        ...
        sw_js = _get("https://dapi.kakao.com/v2/local/search/category.json", {"category_group_code": "SW8", "x": lon, "y": lat, "radius": 900, "size": 1, "sort": "distance"})
        if sw_js and sw_js.get("documents"):
            subway_name, subway_line = _parse_kakao_subway_doc(sw_js["documents"][0])
        for r in [900, 1200, 1500]:
            js = _get("https://dapi.kakao.com/v2/local/search/keyword.json", {"query": "버스정류장", "x": lon, "y": lat, "radius": r, "size": 1, "sort": "distance"})
            if js and js.get("documents"):
                bus_name = _nfc(js["documents"][0].get("place_name"))
                break
...
def _line_station_text(station: str, line: str) -> str:
    ...
    if line and (line.replace(",", "") in station.replace(",", "")): return station
    return f"{station} {line}".strip()
