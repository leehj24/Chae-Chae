...
# merged['review_score'] = merged['pop_review_score']
...
#     '총_관광지_수': [len(places)],
#     'region기준_매칭_성공(재작성 review_score)': [merged['review_score'].notna().sum()],
#     'region기준_매칭_실패': [merged['review_score'].isna().sum()],
# })
...
# 열 이름 변경
places.rename(columns={'review_socre': 'review_score'}, inplace=True)

places.to_csv(out_path, index=False, encoding='utf-8-sig')
out_path
